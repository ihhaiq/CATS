...
async def handle_rich_action(query: CallbackQuery) -> None:
    ...
    if action == "wake" and random.random() < 0.2:
        cat["action_notice"] = "😾 القطة تريد النوم أكثر!"
        await update_cat(cat)
        ...
        return
    if sleep_need_percent(cat) >= 85 and action in {"feed", "play", "walk", "talk"}:
    ...
    if action == "feed":
        ...
        cat["last_decay_at"] = datetime.utcnow().isoformat()
        await update_cat(cat)
    ...
    await update_cat(cat)
    if points:
        await award_points(user_id, points, action)
